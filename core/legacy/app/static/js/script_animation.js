...
function createRenderer(structure, frames) {
  ...
  if (frameList.length === 0) {
    console.warn("No frames or blocks found in JSON; nothing to render.");
    return;
  }
  ...
  function animate() {
    renderFrame(structure, frameList[currentFrame]);
...
function structureFromJsonData(data) {
  ...
  // Map to hold frame data; fall back to a single frame using "blocks"
  const frames =
