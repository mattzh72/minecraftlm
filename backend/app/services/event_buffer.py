...
class SessionEventBuffer:
    ...
    The buffer accumulates ALL events for a task's lifetime. Events are never
    cleared during streaming - this eliminates race conditions between producer
    and consumer.

    On resume, subscribers replay from since=0 to rebuild in-progress state.
    The buffer is replaced when a new task starts (via create_new_buffer).
    """
