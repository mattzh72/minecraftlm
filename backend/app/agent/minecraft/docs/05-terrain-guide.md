...
def build_structure() -> dict:
    ...
    # Generate 128x128 terrain (default biome: "plains")
...
def build_structure() -> dict:
    terrain = create_terrain(
        ...
        seed=42,
        biome="taiga",
        tree_density=0.6,  # <1 fewer trees, >1 more trees
    )
    terrain.generate()
...
The terrain generator currently supports **one land biome per Terrain** via `create_terrain(..., biome=...)` or `TerrainConfig(biome=...)`:

- `plains` (default): grass/dirt/stone with plains vegetation
- `forest`: same ground as plains, denser mixed trees
- `desert`: sand/sandstone/stone with sparse cactus + dead bushes
- `snowy_plains`: snowy grass with thin snow overlay and snowy spruce variants
- `taiga`: podzol/dirt/stone with spruce/pine variants
...
config = TerrainConfig(
    ...
    liquid_block_id="minecraft:water",  # Liquid for oceans/lakes/rivers (e.g. water, lava, ice)
...
The land surface stack depends on `biome` (top → bottom):

- `plains` / `forest`: grass block → dirt → stone
- `desert`: sand → sandstone → stone
- `snowy_plains`: grass block (snowy) → dirt → stone (+ thin snow overlay)
...
# Add a spruce (useful for taiga/snowy scenes)
spruce = generate_spruce_tree(55, height, 55, catalog=catalog, seed=456)
scene.add(spruce)
...
terrain.add_mountain(
    ...
    block_id="minecraft:stone",   # Optional: mountain material (defaults to stone)
    snow=True,           # Add snow cap (default: True)
    snowy="top",         # Optional: thin snow layer ("none" | "top" | "all")
...
terrain.add_ridge(
    ...
    block_id="minecraft:stone",   # Optional: ridge material
    snow=True,           # Add snow cap
    snowy="none",        # Optional: thin snow layer ("none" | "top" | "all")
)
...
terrain.add_plateau(
    ...
    block_id="minecraft:stone",   # Optional: plateau material
...
| All | `block_id` | `"minecraft:stone"` | Base material block id for mountain/ridge/plateau |
| All | `snowy` | `"none"` | Thin snow layer: `"none"`, `"top"`, or `"all"` |
...
Notes:
- `snow=True` places full `minecraft:snow_block` on the cap above the snow line.
- `snowy="top"` adds a thin `minecraft:snow` layer on exposed surfaces near the top; `snowy="all"` covers the whole mountain surface with a thin snow layer.

### Block Types

Mountains use proper Minecraft block layers:
- **Snow cap**: `minecraft:snow_block` at elevations above snow line
- **Snow layer**: `minecraft:snow` (thin layer) when `snowy="top"` or `snowy="all"`
- **Mountain material**: controlled by `block_id` (defaults to `minecraft:stone`)
...
By default, water features use `minecraft:water`. You can change the liquid block globally with `liquid_block_id` (and optionally override it per lake/river/crater/valley fill):

- Water: `liquid_block_id="minecraft:water"` (default)
- Lava: `liquid_block_id="minecraft:lava"`
- Frozen: `liquid_block_id="minecraft:ice"` (renders as solid ice columns)
...
terrain = create_terrain(
    ...
    liquid_block_id="minecraft:water",  # Or "minecraft:lava", "minecraft:ice"
    seed=42,
)
...
terrain.add_lake(
    ...
    liquid_block_id="minecraft:water",  # Optional override per-lake
)
...
terrain.add_river(
    ...
    liquid_block_id="minecraft:water",  # Optional override per-river
)
